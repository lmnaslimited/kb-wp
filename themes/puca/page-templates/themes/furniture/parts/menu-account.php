<?php if( !(defined('PUCA_WOOCOMMERCE_CATALOG_MODE_ACTIVED') && PUCA_WOOCOMMERCE_CATALOG_MODE_ACTIVED) && has_nav_menu( 'nav-account' )) : ?>
	...
	<div class="tbay-login">
		...
		<?php if (is_user_logged_in() ) { ?>
			...
			<div class="dropdown">
				<a class="account-button" href="javascript:void(0);"><i class="icon-user icons"></i><span class="hidden-xs"><?php echo esc_html( $current_user->display_name); ?>!</span></a>
		...
		<?php } elseif( !(defined('PUCA_WOOCOMMERCE_CATALOG_MODE_ACTIVED') && PUCA_WOOCOMMERCE_CATALOG_MODE_ACTIVED) && defined('PUCA_WOOCOMMERCE_ACTIVED') && PUCA_WOOCOMMERCE_ACTIVED && !empty(get_option('woocommerce_myaccount_page_id')) ) { ?>  
				<a href="<?php echo esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ); ?>" class="account-button" title="<?php esc_html_e('Login / Sign up','puca'); ?>"><i class="icon-user"></i></a>          	
		<?php } ?> 
