		...
		<div class="mcart-border"> 
						...
						if($_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters('woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key)) {
							...
							<li id="mcitem-<?php echo esc_attr($_id);?>-<?php echo esc_attr($cart_item_key); ?>">
								...
								<div class="product-details">
									...
									<a class="product-name" href="<?php echo esc_url( $product_permalink ); ?>"><?php echo trim($product_name); ?></a>
									...
									<?php echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
									    '<a href="%s" class="remove remove_from_cart_button" aria-label="%s" data-product_id="%s" data-product_sku="%s" data-cart_item_key="%s">&#215;</a>',
			...
			<?php if(sizeof(WC()->cart->get_cart()) > 0) : ?>
				<div class="group-button">
					<p class="total"><?php esc_html_e('Cart total', 'puca'); ?>: <?php echo WC()->cart->get_cart_subtotal(); ?></p>

					<?php do_action('woocommerce_widget_shopping_cart_before_buttons'); ?>

					<p class="buttons">
						<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="button view-cart"><?php printf( esc_html__( 'View Cart(%s)', 'puca' ), WC()->cart->get_cart_contents_count() ); ?><i class="icon-arrow-right icons"></i></a>	
						<a href="<?php echo esc_url( wc_get_checkout_url() );?>" class="button checkout"><?php esc_html_e('Checkout', 'puca'); ?><i class="icon-lock"></i></a>
					</p>
				</div>
			<?php endif; ?>
