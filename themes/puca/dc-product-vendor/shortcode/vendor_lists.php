...
<div id="wcmp-store-conatiner">
        ...
        <form name="vendor_list_sort" method="post">
            ...
            <div class="wcmp-store-map-filter">
                <div class="wcmp-inp-wrap wcmp-email">
                    <input type="text" name="locationText" id="locationText" placeholder="<?php esc_html_e('Enter Address', 'puca'); ?>" value="<?php echo isset($request['locationText']) ? $request['locationText'] : ''; ?>">
                </div>
                <div class="wcmp-inp-wrap wcmp-within">
                    <select name="radiusSelect" id="radiusSelect">
                        <option value=""><?php esc_html_e('Within', 'puca'); ?></option>
                        <?php if($radius) :
                ...
                <div class="wcmp-inp-wrap wcmp-distanceSelect">
                    <select name="distanceSelect" id="distanceSelect">
                        ...
                        <option value="M" <?php echo selected( $selected_distance, "M", false ); ?>><?php esc_html_e('Miles', 'puca'); ?></option>
                        <option value="K" <?php echo selected( $selected_distance, "K", false ); ?>><?php esc_html_e('Kilometers', 'puca'); ?></option>
                        <option value="N" <?php echo selected( $selected_distance, "N", false ); ?>><?php esc_html_e('Nautical miles', 'puca'); ?></option>
                ...
                <input type="submit" name="vendorListFilter" value="<?php esc_html_e('Submit', 'puca'); ?>">
            </div>
        </form>
        <?php endif; ?>
        <div class="wcmp-store-map-pagination">
            <p class="wcmp-pagination-count wcmp-pull-right">
                ...
                if ( $vendor_total <= $per_page || -1 === $per_page ) {
                        ...
                        printf( _n( 'Viewing the single vendor', 'Viewing all %d vendors', $vendor_total, 'puca' ), $vendor_total );
                } else {
                        ...
                        printf( _nx( 'Viewing the single vendor', 'Viewing %1$d&ndash;%2$d of %3$d vendors', $vendor_total, 'with first and last result', 'puca' ), $first, $last, $vendor_total );
                }
                ?>
            </p>
                ...
                <div class="vendor_sort">
                    <select class="select short" id="vendor_sort_type" name="vendor_sort_type">
                        ...
                        $vendor_sort_type = apply_filters('wcmp_vendor_list_vendor_sort_type', array(
                            'registered' => esc_html__('By date', 'puca'),
                            'name' => esc_html__('By Alphabetically', 'puca'),
                            'category' => esc_html__('By Category', 'puca'),
                        ));
                    ...
                    <input value="<?php echo esc_html__('Sort', 'puca'); ?>" type="submit">
                </div>
            </form>
    ...
    <div class="wcmp-store-list-wrap">
        ...
        if ($vendors && is_array($vendors)) {
            ...
            esc_html_e('No vendor found!', 'puca');
        }
        ?>
    </div>
    <!-- pagination --> 
