...
require_once( get_parent_theme_file_path( PUCA_INC . '/merlin-data/wpbakery.php') );
require_once( get_parent_theme_file_path( PUCA_INC . '/merlin-data/elementor.php') );

class Puca_Merlin_Config {
	...
	private function init() {
		$wizard = new Merlin(
			...
			$strings = array(
				'admin-menu'          => esc_html__( 'Theme Setup', 'puca' ),

				/* translators: 1: Title Tag 2: Theme Name 3: Closing Title Tag */
				'title%s%s%s%s'       => esc_html__( '%1$s%2$s Themes &lsaquo; Theme Setup: %3$s%4$s', 'puca' ),
				'return-to-dashboard' => esc_html__( 'Return to the dashboard', 'puca' ),
				'ignore'              => esc_html__( 'Disable this wizard', 'puca' ),

				'btn-skip'                 => esc_html__( 'Skip', 'puca' ),
				'btn-next'                 => esc_html__( 'Next', 'puca' ),
				'btn-start'                => esc_html__( 'Start', 'puca' ),
				'btn-no'                   => esc_html__( 'Cancel', 'puca' ),
				'btn-plugins-install'      => esc_html__( 'Install', 'puca' ),
				'btn-child-install'        => esc_html__( 'Install', 'puca' ),
				'btn-content-install'      => esc_html__( 'Install', 'puca' ),
				'btn-import'               => esc_html__( 'Import', 'puca' ),
				'btn-license-activate'     => esc_html__( 'Activate', 'puca' ),
				'btn-license-skip'         => esc_html__( 'Later', 'puca' ),

				/* translators: Theme Name */
				'license-header%s'         => esc_html__( 'Activate %s', 'puca' ),
				/* translators: Theme Name */
				'license-header-success%s' => esc_html__( '%s is Activated', 'puca' ),
				/* translators: Theme Name */
				'license%s'                => esc_html__( 'Enter your license key to enable remote updates and theme support.', 'puca' ),
				'license-label'            => esc_html__( 'License key', 'puca' ),
				'license-success%s'        => esc_html__( 'The theme is already registered, so you can go to the next step!', 'puca' ),
				'license-json-success%s'   => esc_html__( 'Your theme is activated! Remote updates and theme support are enabled.', 'puca' ),
				'license-tooltip'          => esc_html__( 'Need help?', 'puca' ),

				/* translators: Theme Name */
				'welcome-header%s'         => esc_html__( 'Welcome to %s', 'puca' ),
				'welcome-header-success%s' => esc_html__( 'Hi. Welcome back', 'puca' ),
				'welcome%s'                => esc_html__( 'This wizard will set up your theme, install plugins, and import content. It is optional & should take only a few minutes.', 'puca' ),
				'welcome-success%s'        => esc_html__( 'You may have already run this theme setup wizard. If you would like to proceed anyway, click on the "Start" button below.', 'puca' ),

				'child-header'         => esc_html__( 'Install Child Theme', 'puca' ),
				'child-header-success' => esc_html__( 'You\'re good to go!', 'puca' ),
				'child'                => esc_html__( 'Let\'s build & activate a child theme so you may easily make theme changes.', 'puca' ),
				'child-success%s'      => esc_html__( 'Your child theme has already been installed and is now activated, if it wasn\'t already.', 'puca' ),
				'child-action-link'    => esc_html__( 'Learn about child themes', 'puca' ),
				'child-json-success%s' => esc_html__( 'Awesome. Your child theme has already been installed and is now activated.', 'puca' ),
				'child-json-already%s' => esc_html__( 'Awesome. Your child theme has been created and is now activated.', 'puca' ),

				'plugins-header'         => esc_html__( 'Install Plugins', 'puca' ),
				'plugins-header-success' => esc_html__( 'You\'re up to speed!', 'puca' ),
				'plugins'                => esc_html__( 'Let\'s install some essential WordPress plugins to get your site up to speed.', 'puca' ),
				'plugins-success%s'      => esc_html__( 'The required WordPress plugins are all installed and up to date. Press "Next" to continue the setup wizard.', 'puca' ),
				'plugins-action-link'    => esc_html__( 'Advanced', 'puca' ),

				'import-header'      => esc_html__( 'Import Content', 'puca' ),
				'import'             => esc_html__( 'Let\'s import content to your website, to help you get familiar with the theme.', 'puca' ),
				'import-action-link' => esc_html__( 'Advanced', 'puca' ),

				'ready-header'      => esc_html__( 'All done. Have fun!', 'puca' ),

				/* translators: Theme Author */
				'ready%s'           => esc_html__( 'Your theme has been all set up. Enjoy your new theme by %s.', 'puca' ),
				'ready-action-link' => esc_html__( 'Extras', 'puca' ),
				'ready-big-button'  => esc_html__( 'View your website', 'puca' ),
				'ready-link-1'      => sprintf( '<a href="%1$s" target="_blank">%2$s</a>', 'https://tickets.thembay.com/', esc_html__( 'Ticket System', 'puca' ) ),
				'ready-link-2'      => sprintf( '<a href="%1$s">%2$s</a>', 'https://docs.thembay.com/puca/', esc_html__( 'Documentation', 'puca' ) ),
				'ready-link-3'      => sprintf( '<a href="%1$s">%2$s</a>', 'https://www.youtube.com/c/thembay/', esc_html__( 'Video Tutorials', 'puca' ) ),
				'ready-link-4'      => sprintf( '<a href="%1$s">%2$s</a>', 'https://forums.thembay.com/', esc_html__( 'Forums', 'puca' ) ),
			)
		);
	}
...
- - - - - - - - - - - - - - - - - - - - */";
	...
	public function after_import_setup( $selected_import ) {
		...
		$check_oneclick  = get_option( 'puca_check_oneclick', [] );
		...
		if ( ! isset( $check_oneclick[ $selected_import['home'] ] ) || apply_filters( 'puca_reset_import_rev_sliders', false ) ) {
			$check_oneclick[ $selected_import['home'] ] = true;
			$this->import_revslider( $selected_import['rev_sliders'] );
			update_option( 'puca_check_oneclick', $check_oneclick );
		}
	...
	private function update_option_woof() {
		...
		$settings['by_price']['title_text'] = esc_html__('Price', 'puca');

		/**Categories**/
		$settings['tax']['product_cat'] = '1';
		$settings['show_title_label']['product_cat'] = '1';
		$settings['custom_tax_label']['product_cat'] = esc_html__('Categories', 'puca');

		/**Size**/
		$settings['tax']['pa_size'] = '1';
		$settings['show_title_label']['pa_size'] = '1';
		$settings['custom_tax_label']['pa_size'] = esc_html__('Product Size', 'puca');

		/**Color**/
		$settings['tax']['pa_color'] = '1';
		$settings['show_title_label']['pa_color'] = '1';
		$settings['custom_tax_label']['pa_color'] = esc_html__('Product Color', 'puca');

		/**Tag**/
		$settings['tax']['product_tag'] = '1';
		$settings['show_title_label']['product_tag'] = '1';
		$settings['custom_tax_label']['product_tag'] = esc_html__('Product Tags', 'puca');

		/**Brand**/
		if( class_exists( 'YITH_WCBR' ) ) {
			$settings['tax']['yith_product_brand'] = '1';
			$settings['show_title_label']['yith_product_brand'] = '1';
			$settings['custom_tax_label']['yith_product_brand'] = esc_html__('Brands', 'puca');
		}	
	...
	public function set_demo_menus() {
		...
		switch ($skin) {
			case 'fashion':
				$this->set_demo_menus_fashion();
				break;

			case 'fashion2':
				$this->set_demo_menus_fashion2();
				break;

			case 'supermaket':
				$this->set_demo_menus_supermaket();
				break;

			case 'supermaket2':
				$this->set_demo_menus_supermaket2();
				break;

			case 'furniture':
				$this->set_demo_menus_furniture();
				break;
	...
	public function set_demo_menus_fashion() {
	...
	public function set_demo_menus_fashion2() {
		$main_menu       	= get_term_by( 'name', 'Main Menu', 'nav_menu' );
		$topmenu   			= get_term_by( 'name', 'Top Menu', 'nav_menu' );
		$nav_account   		= get_term_by( 'name', 'Nav Account', 'nav_menu' );
	...
	public function set_demo_menus_supermaket() {
		$main_menu       	= get_term_by( 'name', 'Main Menu', 'nav_menu' );
		$topmenu   			= get_term_by( 'name', 'Top Menu', 'nav_menu' );
		$nav_account   		= get_term_by( 'name', 'Nav Account', 'nav_menu' );
		$nav_category_img   		= get_term_by( 'name', 'Image Category Menu', 'nav_menu' );
		...
		set_theme_mod(
			...
			array(
				...
				'topmenu' 			=> $topmenu->term_id,
				'nav-account' 		=> $nav_account->term_id,
				'nav-category-img' 	=> $nav_category_img->term_id
			)
		);
	}

	public function set_demo_menus_supermaket2() {
		$main_menu       	= get_term_by( 'name', 'Main Menu', 'nav_menu' );
		$topmenu   			= get_term_by( 'name', 'Top Menu', 'nav_menu' );
		$nav_account   		= get_term_by( 'name', 'Nav Account', 'nav_menu' );
		$nav_category_img   = get_term_by( 'name', 'Categories Menu Image', 'nav_menu' );
		$nav_category   	= get_term_by( 'name', 'Categories Menu', 'nav_menu' );
	...
	public function set_demo_menus_furniture() {
		$main_menu       	= get_term_by( 'name', 'Main Menu', 'nav_menu' );
		$topmenu   			= get_term_by( 'name', 'Top Menu', 'nav_menu' );
		$nav_account   		= get_term_by( 'name', 'Nav Account', 'nav_menu' );
		$nav_category   	= get_term_by( 'name', 'Nav Category Menu', 'nav_menu' );
		...
		set_theme_mod(
			...
			array(
				...
				'topmenu' 				=> $topmenu->term_id,
				'nav-account' 			=> $nav_account->term_id,
				'nav-category-menu' 	=> $nav_category->term_id
			)
		);
	}
	...
	private function import_files_wpb_vc(){
		...
		$data = new Puca_Merlin_Wpbakery;
		
		$fashion_v1 		= $data->import_files_wpb_vc_fashion_v1();
		$fashion_v2 		= $data->import_files_wpb_vc_fashion_v2();
		$smarket_v1 		= $data->import_files_wpb_vc_smarket_v1();
		$smarket_v2 		= $data->import_files_wpb_vc_smarket_v2();
		$furniture  		= $data->import_files_wpb_vc_furniture();
		$dokan  			= $data->import_files_wpb_vc_smarket_dokan();
		$wcmp  				= $data->import_files_wpb_vc_smarket_wcmp();
		$wcfm  				= $data->import_files_wpb_vc_smarket_wcfm();
		$wcvendors  		= $data->import_files_wpb_vc_smarket_wcvendors();

		return array_merge($import, $fashion_v1, $fashion_v2, $smarket_v1, $smarket_v2, $furniture, $dokan, $wcmp, $wcfm, $wcvendors);
	}
	
	private function import_files_elementor(){
		$import = array();

		$data = new Puca_Merlin_Elementor;

		$fashion_v1 = $data->import_files_wpb_el_fashion_v1();
		$fashion_v2 = $data->import_files_wpb_el_fashion_v2();
		$smarket_v1 = $data->import_files_wpb_el_smarket_v1();
		$smarket_v2 = $data->import_files_wpb_el_smarket_v2();
		$furniture  = $data->import_files_wpb_el_furniture();

		$dokan  			= $data->import_files_wpb_el_smarket_dokan();
		$wcmp  				= $data->import_files_wpb_el_smarket_wcmp();
		$wcfm  				= $data->import_files_wpb_el_smarket_wcfm();
		$wcvendors  		= $data->import_files_wpb_el_smarket_wcvendors();
