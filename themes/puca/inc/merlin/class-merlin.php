...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
	...
	public function admin_page() {
		...
		$texts = array(
			'something_went_wrong' => esc_html__( 'Something went wrong. Please refresh the page and try again!', 'puca' ),
		);
	...
	public function svg( $args = array() ) {
		...
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'puca' );
		}

		// Define an icon.
		if ( false === array_key_exists( 'icon', $args ) ) {
			return __( 'Please define an SVG icon filename.', 'puca' );
		}
	...
	public function steps() {
		...
		if ( ! empty( $this->import_files ) ) {
			$this->steps['content'] = array(
				'name' => esc_html__( 'Content', 'puca' ),
				'view' => array( $this, 'content' ),
			);
		}

		$this->steps['ready'] = array(
			'name' => esc_html__( 'Ready', 'puca' ),
	...
	<?php
		$this->logger->debug( __( 'The welcome step has been displayed', 'puca' ) );
	}
	...
	protected function license() {
		...
		<?php
		$this->logger->debug( __( 'The license activation step has been displayed', 'puca' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The child theme installation step has been displayed', 'puca' ) );
	}
					...
					<?php foreach ( $required_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							...
							<label for="default_plugins_<?php echo esc_attr( $slug ); ?>">
								...
								<span class="badge">
									<span class="hint--top" aria-label="<?php esc_html_e( 'Required', 'puca' ); ?>">
										<?php esc_html_e( 'req', 'puca' ); ?>
									</span>
								</span>
							</label>
						</li>
	...
	<?php
		$this->logger->debug( __( 'The plugin installation step has been displayed', 'puca' ) );
	}
	...
	protected function content() {
		...
		<div class="merlin__content--transition">
			...
			printf( '<a href="%s" target="_blank" class="merlin-view-demo">' . esc_html__( 'Watch Demo Online', 'puca' ) . '</a>',  $this->import_files[0]['preview_url'] );
			?>
			...
			<?php if ( 1 < count( $this->import_files ) ) : ?>
				...
				<div class="merlin__select-control-wrapper">
					...
					<div class="merlin__select-control-help">
						<span class="hint--top" aria-label="<?php echo esc_attr__( 'Select Demo', 'puca' ); ?>">
	...
	<?php
		$this->logger->debug( __( 'The content import step has been displayed', 'puca' ) );
	}
	...
	<?php
		$this->logger->debug( __( 'The final step has been displayed', 'puca' ) );
	}
	...
	public function generate_child() {
		...
		if ( ! file_exists( $path ) ) {
			...
			$this->logger->debug( __( 'The existing child theme was activated', 'puca' ) );
		...
		$this->logger->debug( __( 'The newly generated child theme was activated', 'puca' ) );
	...
	public function _ajax_activate_license() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Yikes! The theme activation failed. Please try again or contact support.', 'puca' ),
				)
			);
		}
		...
		if ( empty( $_POST['license_key'] ) ) {
			wp_send_json(
				array(
					'success' => false,
					'message' => esc_html__( 'Please add your license key before attempting to activate one.', 'puca' ),
				)
			);
		}
		...
		$this->logger->debug( __( 'The license activation was performed with the following results', 'puca' ), $result );
	...
	protected function edd_activate_license( $license ) {
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				$message = esc_html__( 'An error occurred, please try again.', 'puca' );
			}
		} else {
				...
				switch ( $license_data->error ) {
					...
					case 'expired':
						$message = sprintf(
							...
							esc_html__( 'Your license key expired on %s.', 'puca' ),
							date_i18n( get_option( 'date_format' ), strtotime( $license_data->expires, current_time( 'timestamp' ) ) )
						);
						break;

					case 'revoked':
						$message = esc_html__( 'Your license key has been disabled.', 'puca' );
						break;

					case 'missing':
						$message = esc_html__( 'This appears to be an invalid license key. Please try again or contact support.', 'puca' );
						break;

					case 'invalid':
					case 'site_inactive':
						$message = esc_html__( 'Your license is not active for this URL.', 'puca' );
						break;

					case 'item_name_mismatch':
						/* translators: EDD Item Name */
						$message = sprintf( esc_html__( 'This appears to be an invalid license key for %s.', 'puca' ), $this->edd_item_name );
						break;

					case 'no_activations_left':
						$message = esc_html__( 'Your license key has reached its activation limit.', 'puca' );
						break;
	...
	public function generate_child_functions_php( $slug ) {
		...
		$this->logger->debug( __( 'The child theme functions.php content was generated', 'puca' ) );
	...
	public function generate_child_style_css( $slug, $parent, $author, $version ) {
		...
		$this->logger->debug( __( 'The child theme style.css content was generated', 'puca' ) );
	...
	public function generate_child_screenshot( $path ) {
		...
		if ( ! empty( $screenshot ) && file_exists( $screenshot ) ) {
			...
			$this->logger->debug( __( 'The child theme screenshot was copied to the child theme, with the following result', 'puca' ), array( 'copied' => $copied ) );
		} else {
			$this->logger->debug( __( 'The child theme screenshot was not generated, because of these results', 'puca' ), array( 'screenshot' => $screenshot ) );
		}
	}
	...
	function _ajax_plugins() {
		...
		foreach ( $plugins['activate'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Activating', 'puca' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['update'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Updating', 'puca' ),
				);
				break;
			}
		}
		...
		foreach ( $plugins['install'] as $slug => $plugin ) {
			if ( $_POST['slug'] === $slug ) {
				$json = array(
					...
					'action2'       => - 1,
					'message'       => esc_html__( 'Installing', 'puca' ),
				);
				break;
			}
		}
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'puca' ),
				array(
			...
			$json['message'] = esc_html__( 'Installing', 'puca' );
			wp_send_json( $json );
		} else {
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'puca' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
			...
			wp_send_json(
				array(
					'done'    => 1,
					'message' => esc_html__( 'Success', 'puca' ),
				)
			);
		}
	...
	function _ajax_content() {
		...
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
			$this->logger->error( __( 'The content importer AJAX call failed to start, because of incorrect data', 'puca' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid content!', 'puca' ),
				)
			);
		}
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'puca' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'puca' ),
				array(
	...
	public function _ajax_get_total_content_import_items() {
		if ( ! check_ajax_referer( 'merlin_nonce', 'wpnonce' ) && empty( $_POST['selected_index'] ) ) {
			$this->logger->error( __( 'The content importer AJAX call for retrieving total content import items failed to start, because of incorrect data.', 'puca' ) );

			wp_send_json_error(
				array(
					'error'   => 1,
					'message' => esc_html__( 'Invalid data!', 'puca' ),
				)
			);
		}
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['content'] ) ) {
			$content['content'] = array(
				'title'            => esc_html__( 'Content', 'puca' ),
				'description'      => esc_html__( 'Demo content data.', 'puca' ),
				'pending'          => esc_html__( 'Pending', 'puca' ),
				'installing'       => esc_html__( 'Installing', 'puca' ),
				'success'          => esc_html__( 'Success', 'puca' ),
		...
		if ( ! empty( $import_files['widgets'] ) ) {
			$content['widgets'] = array(
				'title'            => esc_html__( 'Widgets', 'puca' ),
				'description'      => esc_html__( 'Sample widgets data.', 'puca' ),
				'pending'          => esc_html__( 'Pending', 'puca' ),
				'installing'       => esc_html__( 'Installing', 'puca' ),
				'success'          => esc_html__( 'Success', 'puca' ),
		...
		if ( ! empty( $import_files['sliders'] ) ) {
			$content['sliders'] = array(
				'title'            => esc_html__( 'Revolution Slider', 'puca' ),
				'description'      => esc_html__( 'Sample Revolution sliders data.', 'puca' ),
				'pending'          => esc_html__( 'Pending', 'puca' ),
				'installing'       => esc_html__( 'Installing', 'puca' ),
				'success'          => esc_html__( 'Success', 'puca' ),
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'puca' ),
				'description'      => esc_html__( 'Sample theme options data.', 'puca' ),
				'pending'          => esc_html__( 'Pending', 'puca' ),
				'installing'       => esc_html__( 'Installing', 'puca' ),
				'success'          => esc_html__( 'Success', 'puca' ),
		...
		if ( ! empty( $import_files['redux'] ) ) {
			$content['redux'] = array(
				'title'            => esc_html__( 'Redux Options', 'puca' ),
				'description'      => esc_html__( 'Redux framework options.', 'puca' ),
				'pending'          => esc_html__( 'Pending', 'puca' ),
				'installing'       => esc_html__( 'Installing', 'puca' ),
				'success'          => esc_html__( 'Success', 'puca' ),
		...
		if ( false !== has_action( 'merlin_after_all_import' ) ) {
			$content['after_import'] = array(
				'title'            => esc_html__( 'After import setup', 'puca' ),
				'description'      => esc_html__( 'After import setup.', 'puca' ),
				'pending'          => esc_html__( 'Pending', 'puca' ),
				'installing'       => esc_html__( 'Installing', 'puca' ),
				'success'          => esc_html__( 'Success', 'puca' ),
	...
	public function import_revolution_sliders( $file ) {
		...
		$this->logger->info( __( 'The revolution slider import was executed', 'puca' ) );
	...
	public function pt_importer_new_ajax_request_response_data( $data ) {
		...
		$data['message']  = esc_html__( 'Installing', 'puca' );
		$data['proceed']  = 'true';
	...
	public function after_content_import_setup() {
		...
		if ( $homepage ) {
			...
			$this->logger->debug( __( 'The home page was set', 'puca' ), array( 'homepage_id' => $homepage ) );
		}
		...
		if ( $blogpage ) {
			...
			$this->logger->debug( __( 'The blog page was set', 'puca' ), array( 'blog_page_id' => $blogpage ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		if ( ! empty( $hello_world ) ) {
			...
			$this->logger->debug( __( 'The Hello world post status was set to draft', 'puca' ) );
		}
	}
	...
	public function validate_import_file_info( $import_files ) {
		...
		foreach ( $import_files as $import_file ) {
			if ( ! empty( $import_file['import_file_name'] ) ) {
				...
				$this->logger->warning( __( 'This predefined demo import does not have the name parameter: import_file_name', 'puca' ), $import_file );
			}
		}
